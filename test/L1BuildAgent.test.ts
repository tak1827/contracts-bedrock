...
describe('L1BuildAgent', function () {
  async function deployFixture() {
    ...
    const bL2OutputOracle = await (
      await ethers.getContractFactory('Build_L2OutputOracle')
    ).deploy()
    const bOptimismPortal = await (
      await ethers.getContractFactory('Build_OptimismPortal')
    ).deploy()
    const bL1CrossDomainMessenger = await (
      await ethers.getContractFactory('Build_L1CrossDomainMessenger')
    ).deploy()
    const bSystemConfig = await (
      await ethers.getContractFactory('Build_SystemConfig')
    ).deploy()
    const bL1StandardBridge = await (
      await ethers.getContractFactory('Build_L1StandardBridge')
    ).deploy()
    const bL1ERC721Bridge = await (
      await ethers.getContractFactory('Build_L1ERC721Bridge')
    ).deploy()

    const bAddrList: AddressLike[] = [
  ...
  describe('build', function () {
    it('success: the owner of proxyAdmin is EOA, admin of all proxys are proxyAdmin', async function () {
      ...
      for (let i = 0; i < e.args?.proxys.length; i++) {
        expect(await proxyAdmin.getProxyAdmin(e.args?.proxys[i])).to.equal(
          e.args?.proxyAdmin
        )
      }
      // assert batchInbox
      expect(e.args?.batchInbox).to.equal(
        '0xfF0000000000000000000000000000000084ff00'
      )
    })
